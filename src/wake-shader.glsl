...
layout (location = 1) in float timeToLive;
layout (location = 2) in mat4 transforms[3];
...
void main() {
	...
	if (viewCoords.x > 1.0 || viewCoords.y > 1.0 || viewCoords.z > 1.0) {
		...
		const float foamMask = texture2D(diffuse, texCoords).r * transparency;

		outColor = vec4(mix(sceneColor, vec3(1.0), foamMask), 1.0);
		outNormal = vec4(sceneNormal, 1.0);
		outLighting = mix(sceneLighting, vec4(0.1, 0.8, 1.0, 1.0), foamMask);
	}
}
